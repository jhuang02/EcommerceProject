...
const AUTHENTICATE_QRY = 'SELECT COUNT(*) FROM user WHERE username = ? and password = ?';
  ...
  if (!checkValidParams(params)) {
    ...
    try {
      ...
      if (isUsernameInDatabase === 1) {
        let retrievedData = await db.all(AUTHENTICATE_QRY, params);
